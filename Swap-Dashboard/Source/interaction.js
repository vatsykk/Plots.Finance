...
async function UpdateConfirmation(){
    ...
    if(document.getElementById("ETH").value > 0 && document.getElementById("ETH").value < await (await getBalances() - GasEstimate + 0.00001)){
        openReviewMint();
    }
    else{
...
async function Maximise(){
    ...
    if(bal - GasEstimate > 0){
        document.getElementById("ETH").value = ((bal - GasEstimate)).toFixed(5);
    }
    else{
        document.getElementById("ETH").value = 0;
    }
